...
impl TokioProcessPty {
    pub fn new(config: &PtyConfig) -> Result<Self, PtyError> {
        info!(
            "TokioProcessPty: Creating PTY with command: {:?}, args: {:?}",
...
impl AsyncRead for TokioProcessPty {
    fn poll_read(
        ...
        if let Ok(Some(status)) = self_mut.child.try_wait() {
            debug!(
                "TokioProcessPty: Child process exited with status: {:?}",
                status
            );
...
impl AsyncWrite for TokioProcessPty {
    fn poll_write(
        ...
        // 添加调试日志
        debug!(
            "TokioProcessPty: Polling for write, buffer length: {}",
            buf.len()
        );
        debug!(
            "TokioProcessPty: Writing data: {:?}",
